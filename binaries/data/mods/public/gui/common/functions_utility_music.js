...
function newRandomSound(soundType, soundSubType, soundPrePath)
{
	...
	{
		Engine.Console_Write (sprintf(translate("Failed to find sounds matching '*%(subtype)s*'"), { soundSubType: subtype }));
		return undefined;
	}
	...
	switch (soundType)
	{
		...
		break;
		case "effect":
			Engine.Console_Write(sprintf(translate("am loading effect '*%(path)s*'"), { path: randomSoundPath }));
		break;
		default:
		break;
	}
