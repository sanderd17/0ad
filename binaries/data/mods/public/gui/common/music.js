...
Music.prototype.updateState = function()
{
	if (this.currentState != this.oldState)
	{
		...
		default:
			warn(sprintf(translate("%(functionName)s: Unknown music state: %(state)s"), { functionName: "Music.updateState()", state: this.currentState }));
			break;
		}
	}
};
...
Music.prototype.storeTracks = function(civMusic)
{
	...
	for each (var music in civMusic)
	{
		...
		if (type === undefined)
		{
			warn(sprintf(translate("%(functionName)s: Unrecognized music type: %(musicType)s"), { functionName: "Music.storeTracks()", musicType: music.Type }));
			continue;
		}
